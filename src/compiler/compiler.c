...
static void binary(bool canAssign) {
    ...
    switch (operatorType) {
        ...
        case TOKEN_LEFT_SHIFT:    emitByte(OP_LSHIFT); break;
        case TOKEN_RIGHT_SHIFT:   emitByte(OP_RSHIFT); break;
...
static void number(bool canAssign) {
    ...
    if (parser.previous.length > 2 && parser.previous.start[0] == '0' && parser.previous.start[1] == 'x') {
        value = (double)strtoul(parser.previous.start + 2, NULL, 16);
    } else {
...
static void string(bool canAssign) {
    ...
    for (int i = 0; i < length; i++) {
        char c = parser.previous.start[1 + i];
        if (c == '\\' && i + 1 < length) {
            i++;
            char next = parser.previous.start[1 + i];
            switch (next) {
                case 'n':  buffer[j++] = '\n'; break;
...
static void unary(bool canAssign) {
    ...
    if (operatorType == TOKEN_AMPERSAND) {
        ...
        int arg = resolveLocal(current, &name);
        if (arg != -1) {
            emitBytes(OP_ADDR_LOCAL, (uint8_t)arg);
        } else {
            uint8_t constant = identifierConstant(&name);
            emitBytes(OP_ADDR_GLOBAL, constant);
        }
        return;
    }
...
ParseRule rules[] = {
  ...
  [TOKEN_AMPERSAND]     = {unary,    binary, PREC_BIT_AND},
